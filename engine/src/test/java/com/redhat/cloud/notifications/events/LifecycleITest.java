...
public class LifecycleITest {
    ...
    void test() {
        ...
        Endpoint emailEndpoint = createEmailSubscriptionEndpoint(accountId);
        addBehaviorGroupAction(defaultBehaviorGroup, emailEndpoint);
        ...
        retry(() -> checkEndpointHistory(accountId, endpoint1, 1, true));
        retry(() -> checkEndpointHistory(accountId, endpoint2, 1, true));
        retry(() -> checkEndpointHistory(accountId, emailEndpoint, 0, true));
        ...
        retry(() -> checkEndpointHistory(accountId, endpoint1, 2, true));
        retry(() -> checkEndpointHistory(accountId, endpoint2, 2, true));
        retry(() -> checkEndpointHistory(accountId, endpoint3, 1, false));
        retry(() -> checkEndpointHistory(accountId, emailEndpoint, 0, true));
        ...
        // Let's check the notifications history again.
        retry(() -> checkEndpointHistory(accountId, endpoint1, 3, true));
        retry(() -> checkEndpointHistory(accountId, endpoint2, 3, true));
        retry(() -> checkEndpointHistory(accountId, endpoint3, 2, false));
        retry(() -> checkEndpointHistory(accountId, emailEndpoint, 1, true));
        ...
        // Let's check the notifications history again.
        retry(() -> checkEndpointHistory(accountId, endpoint1, 4, true));
        retry(() -> checkEndpointHistory(accountId, endpoint2, 4, true));
        retry(() -> checkEndpointHistory(accountId, endpoint3, 3, false));
        retry(() -> checkEndpointHistory(accountId, emailEndpoint, 2, true));
    ...
    private Endpoint createEmailSubscriptionEndpoint(String accountId) {
        return createEndpoint(accountId, EMAIL_SUBSCRIPTION, "Email endpoint", "System email endpoint", new EmailSubscriptionProperties());
    }
    ...
    private boolean checkEndpointHistory(String accountId, Endpoint endpoint, int expectedHistoryEntries, boolean expectedInvocationResult) {
        return sessionFactory.withStatelessSession(statelessSession -> {
            return statelessSession.createQuery("FROM NotificationHistory WHERE invocationResult = :invocationResult", NotificationHistory.class)
