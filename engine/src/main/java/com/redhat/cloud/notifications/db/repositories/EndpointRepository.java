...
public class EndpointRepository {
    ...
    public Endpoint getOrCreateDefaultEmailSubscription(String accountId) {
        ...
        List<Endpoint> endpoints = statelessSession.createQuery(query, Endpoint.class)
        ...
        statelessSession.insert(endpoint.getProperties());
        return endpoint;
    }
    ...
    private <T extends EndpointProperties> void loadTypedProperties(Class<T> typedEndpointClass, Set<Endpoint> endpoints, EndpointType type) {
        ...
        if (endpointsMap.size() > 0) {
            List<T> propList = statelessSession.createQuery("FROM " + typedEndpointClass.getSimpleName() + " WHERE id IN (:ids)", typedEndpointClass)
